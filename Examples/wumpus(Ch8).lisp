...
(defun make-city-edges ()
  (let* ((nodes (loop for i from 1 to *node-num*
	 ...
	 (cops (remove-if (complement (lambda (x)
...
(defun with-one (a b edge-alist)
  (member b (neighbors a edge-alist)))

(defun with-two (a b edge-alist)
  (or (with-one a b edge-alist)
      (some (lambda (x)
    ...
    (loop for n from 1 to *node-num*
       collect (append (list n)
		       (cond ((eql n wumpus) '(wumpus))
			     ((with-two n wumpus edge-alist) '(blood!)))
		       (cond ((member n glow-worms)
			      '(glow-worms))
			     ((some (lambda (worm)
				      (with-one n worm edge-alist))
				    glow-worms)
			      '(lights!)))
...
(defun find-empty-node ()
  (let ((node (random-node)))
    (if (cdr (assoc node *congestion-city-nodes*))
	(find-empty-node)
	node)))
